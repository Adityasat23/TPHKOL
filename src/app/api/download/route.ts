  ...
  try {
    ...
    if (url.includes('instagram.com/')) {
      try {
        const res = await fetch('https://co.wuk.sh/api/json', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...BROWSER_HEADERS, 'Accept': 'application/json' },
          body: JSON.stringify({ url: url })
        });
        const data = await res.json();
        if (data && data.url) return NextResponse.json({ title: 'Instagram Media', cover: '', play: data.url, music: '' });
      } catch (e) { console.log("❌ Instagram Engine 1 Error"); }

      // Engine 2: SnapInsta API / alternative (we'll just use Cobalt for now or error)
      return NextResponse.json({ error: 'Gagal ekstrak Instagram. Pastikan akun tidak di-private.' }, { status: 400 });
    }
