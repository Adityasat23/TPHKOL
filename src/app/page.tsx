...
const WA_COLORS = [
  ...
  const handleSelectProduct = (product: { name: string; image: string }) => {
    ...
    setProductImage(product.image); // Gambar langsung diganti berdasarkan source lokal
    setShowSuggestions(false);
  };
  // ==========================================
  ...
  return (
    <main className="min-h-screen bg-[#0F1115] text-[#F3F4F6] py-12 px-4 font-sans relative overflow-hidden selection:bg-[#C084FC]/30 selection:text-[#F3F4F6]">
      ...
      {/* --- HERO SECTION --- */}
      <div className="max-w-3xl w-full text-center mb-10 mt-4 relative z-10 mx-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                {showSuggestions && filteredCatalog.length > 0 && (
                  <ul className="absolute z-[100] w-full left-0 mt-2 bg-[#1D212B] border border-[#374151] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] max-h-56 overflow-y-auto custom-scrollbar border-t-[#C084FC]/20 animate-in fade-in slide-in-from-top-2 duration-200">
                    {filteredCatalog.map((item, index) => (
                      ...
                      >
                        <img 
                          src={item.image} 
                          alt={item.name} 
                          className="w-9 h-9 rounded object-cover border border-[#374151]" 
                          onError={(e) => { (e.target as HTMLImageElement).src = DEFAULT_PRODUCT; }} 
                        />
              ...
              {(productLayout === 'tiktok-portrait' || productLayout === 'tiktok-landscape') && (
                <div ref={productPreviewRef} style={{ backgroundColor: '#ffffff', borderRadius: '12px', overflow: 'hidden', fontFamily: 'Arial, sans-serif', width: productLayout === 'tiktok-portrait' ? '300px' : '480px', display: 'flex', flexDirection: productLayout === 'tiktok-portrait' ? 'column' : 'row', boxShadow: '0 15px 40px rgba(0,0,0,0.5)' }}>
                  ...
                  <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: '12px', gap: '8px', flexWrap: 'nowrap' }}>
      ...
      {activeTab === 'comment' && (
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-300">
          <div className="bg-[#171A21] border border-[#262A35] rounded-3xl p-8 space-y-6 shadow-2xl h-fit">
            ...
            <button onClick={exportCommentImage} className="w-full bg-gradient-to-r from-[#A78BFA] to-[#C084FC] hover:from-[#B79BFB] hover:to-[#D09DFD] text-[#0F1115] font-bold py-4 rounded-xl shadow-lg shadow-[#A78BFA]/10 hover:shadow-xl hover:shadow-[#A78BFA]/20 transition-all duration-300 active:scale-[0.98]">
              📸 DOWNLOAD IMAGE
            </button>
          </div>

          <div className="bg-[#0F1115] border border-[#262A35] rounded-3xl p-10 flex items-center justify-center min-h-[500px] overflow-hidden shadow-inner relative">
      ...
      {activeTab === 'wa' && (
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-300">
          ...
          <div className="bg-[#0F1115] border border-[#262A35] rounded-3xl p-6 flex items-center justify-center min-h-[500px] overflow-hidden shadow-inner relative">
             <div className="absolute inset-0 opacity-[0.05] bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:16px_16px]"></div>
      ...
      {activeTab === 'downloader' && (
        <div className="w-full max-w-2xl mx-auto bg-[#171A21] shadow-2xl rounded-3xl p-8 border border-[#262A35] animate-in fade-in duration-300">
      ...
      <footer style={{ marginTop: '80px', paddingTop: '40px', paddingBottom: '30px', textAlign: 'center', width: '100%', maxWidth: '1152px', borderTop: '1px solid rgba(255,255,255,0.05)', zIndex: 10, margin: '0 auto' }}>
